...
BOOL CMbr::OnInitDialog() {
    ...
    CString csBaseSize;
    csBaseSize.Format(_T("Start: %I64i, Size: %I64i"), m_lba, (DWORD64)1);
...
BOOL CMbr::OnHelpInfo(HELPINFO* pHelpInfo) {
    ...
    ShellExecute(AfxGetApp()->m_pMainWnd->m_hWnd, _T("open"),
        url + "mbr.html",
...
bool CMbr::Exists(DWORD64 LBA) {
    ...
    entry = (MBR_PART_ENTRY*)&m_buffer[512 - 2 - (4 * 16)];
    for (i = 0; i < 4; i++) {
    ...
    if (m_exists) {
        for (i = 0; i < 4; i++) {
            ...
            m_Pages[i].m_Title.Format(_T("Entry %i"), i);
    ...
    m_id_sig.Format(_T("0x%08X"), *(DWORD*)&m_buffer[512 - 2 - (4 * 16) - 2 - 4]);
    m_id_zero.Format(_T("0x%04X"), *(WORD*)&m_buffer[512 - 2 - (4 * 16) - 2]);
...
void CMbr::UpdateEntry(CMbrEntry* Entry, int index) {
    ...
    entry = (MBR_PART_ENTRY*)(&m_buffer[512 - 2 - (4 * 16)] + (16 * index));
    Entry->m_boot_id.Format(_T("0x%02X"), entry->boot_id);
    Entry->m_begin_cyl.Format(_T("0x%02X"), entry->start.cyl);
    Entry->m_begin_head.Format(_T("0x%02X"), entry->start.head);
    Entry->m_begin_sect.Format(_T("0x%02X"), entry->start.sector);
    Entry->m_sys_id.Format(_T("0x%02X"), entry->sys_id);
    Entry->m_end_cyl.Format(_T("0x%02X"), entry->end.cyl);
    Entry->m_end_head.Format(_T("0x%02X"), entry->end.head);
    Entry->m_end_sect.Format(_T("0x%02X"), entry->end.sector);
    Entry->m_start_lba.Format(_T("%u"), entry->start_lba);
    if (entry->sectors < 0xFFFFFFFF)
        Entry->m_size.Format(_T("%u"), entry->sectors);
    else // if max sectors, show 0xFFFFFFFF.  It is easier to see than 4,294,967,295
        Entry->m_size.Format(_T("0x%08X"), entry->sectors);
}
...
void CMbr::OnExtractMbr() {
    ...
    odlg.m_ofn.lpstrTitle = _T("Extract MBR to file");
    ...
    if (file.Open(odlg.GetNextPathName(pos), CFile::modeCreate | CFile::modeWrite | CFile::typeBinary | CFile::shareExclusive, NULL) != 0) {
        ...
        file.Close();
        free(buffer);
        AfxMessageBox(_T("MBR Code Extracted Successfully..."));
    }
    else
        AfxMessageBox(_T("Error Creating File..."));
}
...
void CMbr::OnUpdateMbr() {
    ...
    // use a default path?
    csPath = AfxGetApp()->GetProfileString(_T("Settings"), _T("DefaultMBRPath"), NULL);
    if (!csPath.IsEmpty())
        odlg.m_ofn.lpstrInitialDir = csPath;
    odlg.m_ofn.lpstrTitle = _T("Update MBR Code Image File");
    ...
    if (file.Open(odlg.GetNextPathName(pos), CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite, NULL) == 0) {
        AfxMessageBox(_T("Error Opening Image File..."));
        return;
    }
    ...
    if (sectors > 1) {
        ...
        offset = _ttoi(dlg->Gpt.m_entry_offset.GetString());
        if (dlg->Gpt.m_exists && (sectors > offset)) {
            if (AfxMessageBox(_T("New MBR code will overwrite GPT Entries!  Continue?"), MB_YESNO, 0) == IDYES) {
...
void CMbr::OnPrependMBR() {
    ...
    if (AfxMessageBox(_T("This will move all sectors forward and write a MBR at LBA 0!  Continue?"), MB_YESNO, 0) != IDYES)
        return;
    ...
    if (count < 1) {
        AfxMessageBox(_T("Count must be at least 1"));
        return;
    }
    ...
    entry = (MBR_PART_ENTRY*)&m_buffer[446];
