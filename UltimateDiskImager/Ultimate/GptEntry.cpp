...
S_ATTRIBUTES gpt_entry_attrbs[] = {
    //            |                               | <- max (col 67)
{           (1 << 0),           (1 << 0),  0, _T("Required Partition")             , {-1, } },
{           (1 << 1),           (1 << 1),  1, _T("Hidden Partition")               , {-1, } },
{           (1 << 2),           (1 << 2),  2, _T("Legacy BIOS Bootable")           , {-1, } },
{ ((DWORD64)1 << 60), ((DWORD64)1 << 60),  3, _T("Read-only")                      , {-1, } },
{ ((DWORD64)1 << 61), ((DWORD64)1 << 61),  4, _T("Shaddow")                        , {-1, } },
{ ((DWORD64)1 << 62), ((DWORD64)1 << 62),  5, _T("Hidden")                         , {-1, } },
{ ((DWORD64)1 << 63), ((DWORD64)1 << 63),  6, _T("do not automount")               , {-1, } },
...
void CGptEntry::OnAttribute() {
    ...
    if (dlg.DoModal() == IDOK) {
        cs.Format(_T("0x%016I64X"), dlg.m_attrib);
...
void CGptEntry::OnPlusMinus() {
    ...
    DWORD64 start = _ttoi64(page->m_start_lba);
    DWORD64 last = _ttoi64(page->m_last_lba);
    DWORD64 size = last - start + 1;

    // get an absolute value to resize to
    CUltimageResize resize;
    resize.m_cur_size.Format(_T("%I64i"), size);
    resize.m_new_size = "";
    ...
    if ((differ > 0) || (AfxMessageBox(_T("Remove sectors from image?"), MB_YESNO, 0) == IDYES)) {
        if (differ > 0)
        ...
        for (int i = cur_tab + 1; i < tab_count; i++) {
            ...
            val = _ttol(p->m_start_lba);
            p->m_start_lba.Format(_T("%i"), val + differ);
            val = _ttol(p->m_last_lba);
            p->m_last_lba.Format(_T("%i"), val + differ);
        }

        // update the 'last usable' value on the GPT dialog
        val = _ttol(dlg->Gpt.m_last_lba);
    ...
    val = _ttol(page->m_last_lba);
    page->m_last_lba.Format(_T("%i"), val + differ);
    ...
    // reload the image?
    if (AfxMessageBox(_T("Reload image?"), MB_YESNO, 0) == IDYES)
        SendMessage(WM_COMMAND, ID_FILE_RELOAD, 0);
    else
        AfxMessageBox(_T("Save all data and reload the image to show corrected values"));
}
