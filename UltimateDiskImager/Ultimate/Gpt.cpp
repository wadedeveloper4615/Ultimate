...
BOOL CGpt::OnInitDialog() {
    ...
    CString csBaseSize;
    csBaseSize.Format(_T("Start: %I64i, End: %I64i"), convert64(m_first_lba), convert64(m_last_lba));
...
BOOL CGpt::OnHelpInfo(HELPINFO* pHelpInfo) {
    CString url = AfxGetApp()->GetProfileString(_T("Settings"), _T("DefaultHelpURL"), NULL);
    ShellExecute(AfxGetApp()->m_pMainWnd->m_hWnd, _T("open"), url + "gpt.html",  NULL, NULL, SW_SHOWNORMAL);
    return TRUE;
}
...
bool CGpt::Exists(DWORD64 LBA) {
    ...
    memcpy(&m_hdr, m_buffer, sizeof(S_GPT_HDR));
    ...
    if (m_exists) {
        ...
        if (dword != crc32(&m_hdr, m_hdr.hdr_size)) {
            if (AfxMessageBox(_T("Found incorrect CRC on Header.  Continue?"), MB_YESNO, 0) != IDYES) {
                m_exists = FALSE;
                return FALSE;
            }
        }
    ...
    if (m_exists) {
        m_entry_buffer = (S_GPT_ENTRY*)malloc((m_hdr.entries * m_hdr.entry_size) + (dlg->m_sect_size - 1));
        dlg->ReadFromFile(m_entry_buffer, m_hdr.entry_offset, ((m_hdr.entries * m_hdr.entry_size) + (dlg->m_sect_size - 1)) / dlg->m_sect_size);

        m_gpt_sig.Format(_T("%02X %02X %02X %02X %02X %02X %02X %02X"),
            m_hdr.sig[0], m_hdr.sig[1], m_hdr.sig[2], m_hdr.sig[3],
            m_hdr.sig[4], m_hdr.sig[5], m_hdr.sig[6], m_hdr.sig[7]);
        m_sig_chars.Format(_T("%c%c%c%c%c%c%c%c"),
            m_hdr.sig[0], m_hdr.sig[1], m_hdr.sig[2], m_hdr.sig[3],
            m_hdr.sig[4], m_hdr.sig[5], m_hdr.sig[6], m_hdr.sig[7]);
        m_gpt_version.Format(_T("0x%08X"), m_hdr.version);
        m_version_num.Format(_T("%i.%i"), m_hdr.version >> 16, m_hdr.version & 0xFFFF);
        m_hdr_size.Format(_T("%i"), m_hdr.hdr_size);
        m_crc32.Format(_T("0x%08X"), m_hdr.crc32); // only bytes 0 -> hdr_size are checked
        m_rsvd.Format(_T("0x%08X"), m_hdr.resv0);
        m_primary_lba.Format(_T("%I64i"), m_hdr.primary_lba);
        m_backup_lba.Format(_T("%I64i"), m_hdr.backup_lba);
        m_first_lba.Format(_T("%I64i"), m_hdr.first_usable);
        m_last_lba.Format(_T("%I64i"), m_hdr.last_usable);
        GUID_Format(m_gpt_guid, &m_hdr.guid);
        m_entry_offset.Format(_T("%I64i"), m_hdr.entry_offset);
        m_entry_count.Format(_T("%i"), m_hdr.entries);
        m_entry_crc32.Format(_T("0x%08X"), m_hdr.crc32_entries);
        m_entry_size.Format(_T("%i"), m_hdr.entry_size);
        ...
        for (unsigned i = 0; i < m_hdr.entries && i < MAX_GPT_ENTRIES; i++) {
            ...
            if (dlg->IsDlgButtonChecked(IDC_IGNORE_MT_GPT_ENTRIES)) {
                struct _S_GPT_ENTRY* entries = (struct _S_GPT_ENTRY*)m_entry_buffer;
                if ((entries[i].first_lba == 0) && (entries[i].last_lba == 0))
                    continue;
            }
            m_Pages[p].Construct(IDD_GPT_ENTRY, 0);
            m_Pages[p].m_Title.Format(_T("Entry %i"), p);
            if (CheckEntry(i))
                m_Pages[p].m_Title += " !";
...
void CGpt::UpdateEntry(CGptEntry* Entry, int index, const int entry_size) {
    struct _S_GPT_ENTRY* entry = (struct _S_GPT_ENTRY*)((BYTE*)m_entry_buffer + (index * entry_size));

    GUID_Format(Entry->m_guid_type, &entry->guid_type);
    GUID_Format(Entry->m_guid, &entry->guid);
    Entry->m_start_lba.Format(_T("%I64i"), entry->first_lba);
    Entry->m_last_lba.Format(_T("%I64i"), entry->last_lba);
    Entry->m_attrib.Format(_T("0x%016I64X"), entry->attribute);
...
BOOL CGpt::CheckEntry(int index) {
    ...
    for (i = 0; i < index; i++) {
        if (((entries[index].first_lba >= entries[i].first_lba) && (entries[index].first_lba <= entries[i].last_lba)) ||
            ...
            cs.Format(_T("GPT: Entry #%i overlaps Entry #%i!"), index, i);
            AfxMessageBox(cs);
            ret = TRUE;
        }
    }
...
void CGpt::GetGPTEntries() {
    ...
    for (int i = 0; i < m_gpt_entries; i++) {
        S_GPT_ENTRY* entry = (S_GPT_ENTRY*)((BYTE*)m_entry_buffer + (i * m_hdr.entry_size));
        ...
        //FIX MultiByteToWideChar(CP_ACP, 0, m_Pages[i].m_name, -1, (LPWSTR)entry->name, 72);
    }
}
...
void CGpt::OnGptApply() {
    ...
    GetGPTHdr();
    memcpy(m_buffer, &m_hdr, sizeof(S_GPT_HDR));
...
void CGpt::OnCrcButton() {
    ...
    cs.Format(_T("0x%08X"), m_hdr.crc32);
...
void CGpt::OnEcrcButton() {
    ...
    if (AfxMessageBox(_T("Update Header CRC?"), MB_YESNO, 0) == IDYES)
        OnCrcButton();
}
...
void CGpt::OnKillfocusGptGuid() {
    ...
    if (!GUID_CheckFormat(cs)) {
        AfxMessageBox(_T("GUID has illegal format\r\n"
...
void CGpt::OnGPTBackup() {
    ...
    if (AfxMessageBox(_T("Updating the Backup GPT and entries will write\ncurrent values to primary locations too.\nContinue?"), MB_YESNO, 0) != IDYES)
        return;
    ...
    S_GPT_HDR* hdr = (S_GPT_HDR*)m_buffer;
    ...
    gpt_entries_size = ((hdr->entries * sizeof(S_GPT_ENTRY)) + (dlg->m_sect_size - 1)) / dlg->m_sect_size;
...
void CGpt::OnGPTFromBackup() {
    ...
    if (AfxMessageBox(_T("Restore Primary GPT Header and Entries from Backup LBA?\n(Make sure 'BackupLBA' is valid)"), MB_YESNO, 0) != IDYES)
        return;
    ...
    if (!CheckGPT(m_hdr.backup_lba, FALSE))
        if (AfxMessageBox(_T("Backup GPT isn't valid. Continue?"), MB_YESNO, 0) != IDYES)
            return;
    ...
    gpt_entries_size = ((hdr->entries * sizeof(struct _S_GPT_ENTRY)) + (dlg->m_sect_size - 1)) / dlg->m_sect_size;
    temp = hdr->backup_lba;
...
void CGpt::OnGPTTotalCheck() {
    ...
    if (cur_crc == org_crc)
        cs.Format(_T("Checking Header CRC: Pass (0x%08X)\r\n"), cur_crc);
    else
        cs.Format(_T("Checking Header CRC: Fail (0x%08X != 0x%08X)\r\n"), org_crc, cur_crc);
    csInfo += cs;
    modeless.SetDlgItemText(IDC_EDIT, csInfo);
    modeless.GetDlgItem(IDC_EDIT)->UpdateWindow();
    ...
    if (hdr->version != 0x00010000) {
        cs.Format(_T("*Error in Revision value: 0x%08X (%i.%i)\r\n"), hdr->version, hdr->version >> 16, hdr->version & 0xFFFF);
        error_cnt++;
    }
    else
        cs.Format(_T("Found Revision %i.%i\r\n"), hdr->version >> 16, hdr->version & 0xFFFF);
    csInfo += cs;

    // check that the header size is (at lesat) 92
    if ((hdr->hdr_size < 92) || (hdr->hdr_size > dlg->m_sect_size)) {
        cs.Format(_T("*Error in Header Size value: %i\r\n"), hdr->hdr_size);
        error_cnt++;
    }
    else if (hdr->hdr_size != 92)
        cs.Format(_T("Header Size = %i.  Valid, but should be 92.\r\n"), hdr->hdr_size);
    else
        cs.Format(_T("Header Size = %i\r\n"), hdr->hdr_size);
    csInfo += cs;

    // the reserved field should be zero
    if (hdr->resv0) {
        cs.Format(_T("*Reserved DWORD should be zero: (0x%08X)\r\n"), hdr->resv0);
        error_cnt++;
    }

    // the MyLBA (Parimary) should = 1
    if (hdr->primary_lba != 1) {
        cs.Format(_T("*Primary LBA doesn't = 1: (%I64i)\r\n"), hdr->primary_lba);
        error_cnt++;
    }
    ...
    if ((dlg->m_sect_size == 512) && (hdr->first_usable < 34)) {
        ...
        cs.Format(_T("4096-byte sectors:\r\n"
        ...
        cs.Format(_T("First Usable value is within range: %I64i\r\n"), hdr->first_usable);
    csInfo += cs;
    modeless.SetDlgItemText(IDC_EDIT, csInfo);
    modeless.GetDlgItem(IDC_EDIT)->UpdateWindow();
    ...
    if ((dlg->m_sect_size == 512) && (hdr->last_usable > (last_lba - 33))) {
        ...
        cs.Format(_T("Last Usable value is within range: %I64i\r\n"), hdr->last_usable);
    csInfo += cs;
    modeless.SetDlgItemText(IDC_EDIT, csInfo);
    modeless.GetDlgItem(IDC_EDIT)->UpdateWindow();
    ...
    if (hdr->backup_lba != last_lba) {
        cs.Format(_T("*Backup GPT is not at last LBA of disk.\r\n"
            " Should be at %I64i is at %I64i\r\n"), last_lba, hdr->backup_lba);
        error_cnt++;
    }
    else
        cs.Format(_T("Backup GPT LBA is at end of disk: %I64i\r\n"), hdr->backup_lba);
    csInfo += cs;
    ...
    if ((dlg->m_sect_size == 512) && (hdr->entry_offset > (hdr->first_usable - 32))) {
        ...
        cs.Format(_T("*The Entry Offset LBA must be at least 2.\r\n"
            " Current Entry Offset is %I64i\r\n"), hdr->entry_offset);
        error_cnt++;
    }
    else
        cs.Format(_T("Entry Offset value is within range: %I64i\r\n"), hdr->entry_offset);
    csInfo += cs;
    modeless.SetDlgItemText(IDC_EDIT, csInfo);
    modeless.GetDlgItem(IDC_EDIT)->UpdateWindow();
    ...
    if ((hdr->entries * hdr->entry_size) > dword) {
        cs.Format(_T("*Not enough room is allocated for count of entries (%i).\r\n"
            " Space needed = %i bytes, space allocated = %i bytes\r\n"), hdr->entries * hdr->entry_size, dword);
        error_cnt++;
    }
    else
        cs.Format(_T("Found %i bytes for partition entries.\r\n"
    ...
    if (hdr->entry_size & 127) {
        cs.Format(_T("*Entry size of %i is not a multiple of 128.\r\n"), hdr->entry_size);
        entry_size = 128;
        error_cnt++;
    }
    else {
        entry_size = hdr->entry_size;
        cs.Format(_T("Found a valid entry size of %i.\r\n"), entry_size);
    }
    csInfo += cs;
    ...
    if (cur_crc != m_hdr.crc32_entries) {
        cs.Format(_T("*The Entry CRC is not correct.  Should be 0x%08X.\r\n"), cur_crc);
        error_cnt++;
    }
    else
        cs.Format(_T("Entry CRC is correct.\r\n"));
    csInfo += cs;
    ...
    S_GPT_HDR* back = (S_GPT_HDR*)malloc(dlg->m_sect_size);
    ...
    if (memcmp(back, hdr, dlg->m_sect_size) != 0) {
        ...
        for (dword = 0; dword < dlg->m_sect_size; dword++) {
            if (s0[dword] != s1[dword]) {
                cs.Format(_T(" 0x%04X:  0x%02X 0x%02X\r\n"), dword, s0[dword], s1[dword]);
                csInfo += cs;
            }
        }
        error_cnt++;
    ...
    cs.Format(_T("Comparing %i Primary and Backup Partition Entries.\r\n"), dword1);
    csInfo += cs;
    //modeless.SetDlgItemText(IDC_EDIT, csInfo);
    //modeless.GetDlgItem(IDC_EDIT)->UpdateWindow();
    ...
    for (dword = 0; dword < dword1; dword++) {
        if (memcmp(b, p, entry_size) != 0) {
            cs.Format(_T("*Backup Entry #%i doesn't match Primary Entry.\r\n"), dword);
            csInfo += cs;
            /*
            BYTE *s0 = (BYTE *) hdr;
            BYTE *s1 = (BYTE *) back;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (entry[i].sys_id == 0xEE) {
            ...
            if (entry[i].start_lba != 1) {
                cs.Format(_T("*Starting LBA should be 1, is %i\r\n"), entry[i].start_lba);
                csInfo += cs;
                error_cnt++;
            }
    ...
    if (j > 0) {
        cs.Format(_T("Warning: Found %i 'other' used entries.\r\n"), j);
        csInfo += cs;
